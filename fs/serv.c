...
serve_read(envid_t envid, union Fsipc *ipc)
{
    ...
    int r;
    struct OpenFile *f; 
    if ((r = openfile_lookup(envid, req->req_fileid, &f)) < 0) return r;
    ssize_t sz = file_read(f->o_file, ret->ret_buf, req->req_n, f->o_fd->fd_offset);
    if (sz > 0) f->o_fd->fd_offset += sz;
	return sz;
}
